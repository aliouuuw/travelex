...
export default function BookingSuccessPage() {
  ...
  const [paymentStatus, setPaymentStatus] = useState<
    "loading" | "confirmed" | "failed" | "processing"
  >("loading");
  const [finalBookingRef, setFinalBookingRef] = useState<string | null>(
    bookingReference,
  );
  ...
  const reservationData = useReservationByTempBookingId(
    bookingId as Id<"tempBookings"> | null,
  );
  const trip = useTripForBooking(
    tempBooking?.tripId || reservationData?.tripId || "",
  );
  const sendBookingConfirmation = useSendBookingConfirmation();

  // Note: Round trip functionality will be implemented in future versions
  ...
  const isLoadingTrip =
    trip === undefined && !!(tempBooking?.tripId || reservationData?.tripId);
  ...
  const checkBookingStatus = useCallback(async () => {
    ...
    try {
      ...
      if (reservationData) {
        ...
        setFinalBookingRef(reservationData.bookingReference || finalBookingRef);
      } else if (bookingReference === "PROCESSING") {
  ...
  }, [bookingId, bookingReference, reservationData, finalBookingRef]);
  ...
  useEffect(() => {
    ...
    // Check status when data changes
    checkBookingStatus();
  ...
  const handleEmailTicket = async () => {
    if (
      ...
      bookingData.passengerEmail === "Check your email for details"
    ) {
      return;
    }
    ...
    try {
      await sendBookingConfirmation({
        ...
        bookingDetails: {
          selectedSeats: Array.isArray(bookingData.selectedSeats)
      ...
      toast.error(
        "Failed to send booking confirmation email. Please try again.",
      );
    } finally {
        ...
        <Card className="w-full max-w-md">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-muted-foreground mb-6">
              Your payment has been processed successfully. We're still
              finalizing your booking details. This may take a few moments.
            </p>
        ...
        <Card className="w-full max-w-md">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-muted-foreground mb-4">
              There was an issue processing your booking. Please contact support
              or try again.
            </p>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {bookingData.numberOfBags} bag
                    {bookingData.numberOfBags > 1 ? "s" : ""}
                  </p>
                </div>
              </CardContent>
            </Card>
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground">
                        Be at the pickup location 15 minutes before departure
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground">
                        Have your booking reference and valid ID ready
                      </p>
                    </div>
                  </div>
                </div>
