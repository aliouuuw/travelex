-- Final fix for route_template_stations JOIN issues
-- This migration has a later timestamp to override previous incorrect versions
-- The root issue: route_template_stations.route_template_id column doesn't exist
-- Correct relationship: route_template_stations.route_template_city_id -> route_template_cities.id
...
BEGIN
    ...
    SELECT 
        ...
        -- FIXED: Use route_template_city_id instead of route_template_id
        (SELECT jsonb_agg(
            jsonb_build_object(
                'id', rtc.id, 
        ...
        -- FIXED: Use proper JOIN through route_template_cities
        (SELECT jsonb_agg(
            jsonb_build_object(
                'id', ts.id, 
