-- Fix JOIN relationships in get_trip_for_booking function
-- The route_template_stations table has route_template_city_id, not route_template_id
-- We need to join through route_template_cities to access route_template_id
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(p.rating, 0.0) as driver_rating,
        ...
        -- Route cities with stations (correct JOIN)
        (SELECT jsonb_agg(
            jsonb_build_object(
                'id', rtc.id, 
        ...
        WHERE rtc.route_template_id = rt.id) as route_cities,
        
        -- Trip stations (correct JOIN through route_template_cities)
        (SELECT jsonb_agg(
            jsonb_build_object(
                'id', ts.id, 
        ...
        JOIN route_template_cities rtc ON rtc.id = ts.route_template_city_id
        JOIN route_template_stations rts ON rts.route_template_city_id = rtc.id
        WHERE ts.trip_id = t.id) as trip_stations,
        ...
        WHERE rtp.route_template_id = rt.id) as pricing,
