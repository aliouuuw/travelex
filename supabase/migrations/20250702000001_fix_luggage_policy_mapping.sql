-- Fix luggage policy column mapping in get_trip_for_booking function
-- The frontend expects maxBagWeightKg but database has max_weight (total weight)
-- We'll use a standard 20kg per bag limit for the bag-based pricing
...
BEGIN
    ...
    SELECT 
        ...
        -- FIXED: Luggage policy with correct column mapping
        CASE WHEN lp.id IS NOT NULL THEN jsonb_build_object(
            ...
            'maxBags', lp.max_bags,
            'maxBagWeightKg', 20,  -- Standard 20kg per bag limit
