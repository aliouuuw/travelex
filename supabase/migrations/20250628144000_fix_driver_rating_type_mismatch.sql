-- Fix driver_rating type mismatch error
-- The profiles.rating column is float8 (double precision), not decimal

-- Drop all search and booking functions to ensure clean state
...
-- Recreate search_trips_by_segment with correct driver_rating type
...
BEGIN
  ...
  SELECT 
    ...
    t.departure_time::timestamptz,
    t.arrival_time::timestamptz,
    t.available_seats::integer,
    COALESCE(v.capacity, 0)::integer,
    (SELECT jsonb_agg(jsonb_build_object('id', rtc.id, 'cityName', rtc.city_name, 'sequenceOrder', rtc.sequence_order) ORDER BY rtc.sequence_order) FROM route_template_cities rtc WHERE rtc.route_template_id = rt.id)::jsonb,
...
-- Recreate get_trip_for_booking with correct driver_rating type
...
BEGIN
    ...
    SELECT 
        ...
        t.available_seats::INTEGER,
        COALESCE(v.capacity, 0)::INTEGER,
        (SELECT jsonb_agg(jsonb_build_object('id', rtc.id, 'cityName', rtc.city_name, 'sequenceOrder', rtc.sequence_order, 'stations', COALESCE((SELECT jsonb_agg(jsonb_build_object('id', rts.id, 'name', rts.station_name, 'address', rts.station_address)) FROM route_template_stations rts WHERE rts.route_template_id = rt.id AND rts.city_name = rtc.city_name), '[]'::jsonb)) ORDER BY rtc.sequence_order) FROM route_template_cities rtc WHERE rtc.route_template_id = rt.id)::JSONB,
        (SELECT jsonb_agg(jsonb_build_object('id', ts.id, 'cityName', ts.city_name, 'sequenceOrder', ts.sequence_order, 'stationInfo', jsonb_build_object('id', rts.id, 'name', rts.station_name, 'address', rts.station_address), 'isPickupPoint', ts.is_pickup_point, 'isDropoffPoint', ts.is_dropoff_point) ORDER BY ts.sequence_order) FROM trip_stations ts JOIN route_template_stations rts ON rts.id = ts.route_template_station_id WHERE ts.trip_id = t.id)::JSONB,
        (SELECT jsonb_agg(jsonb_build_object('fromCity', rtp.from_city, 'toCity', rtp.to_city, 'price', rtp.price)) FROM route_template_pricing rtp WHERE rtp.route_template_id = rt.id)::JSONB,
        CASE WHEN lp.id IS NOT NULL THEN jsonb_build_object(
            'id', lp.id,
            'name', lp.name,
            'freeWeightKg', lp.free_weight,
