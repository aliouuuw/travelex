-- Fix column reference error in get_trip_for_booking function
-- The error indicates "column rts.route_template_id does not exist" 
-- when it should be "rtc.route_template_id"
...
BEGIN
    ...
    SELECT 
        ...
        -- Fixed: Using rtc.route_template_id instead of rts.route_template_id
        (SELECT jsonb_agg(
            jsonb_build_object(
                'id', rtc.id, 
        ...
        -- Trip stations with correct JOIN reference
        (SELECT jsonb_agg(
            jsonb_build_object(
                'id', ts.id, 
