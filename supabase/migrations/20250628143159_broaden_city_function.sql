-- This migration ensures the get_available_cities function returns both city_name and trip_count
-- as expected by the frontend TypeScript service
...
-- Fix the COALESCE type mismatch in search functions
-- Drop and recreate search_trips_by_segment to fix the text[] vs jsonb issue
...
begin
  ...
  select 
    ...
    coalesce(rtp.price, 0) as segment_price,
